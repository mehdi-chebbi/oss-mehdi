...
export default function ResourceForm() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError.message || "Impossible de charger cette ressource.");
    } finally {
      setLoading(false);
    }
  }, [id, token]);
  ...
  const save = async () => {
    ...
    if (!form.publication_date || form.fields.length === 0) {
      setError("La date de publication et au moins un domaine sont obligatoires.");
      return;
    }
    ...
    try {
      ...
      setError(saveError.message || "Impossible d’enregistrer cette ressource.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl p-8">
      ...
      <h1 className="text-2xl font-bold text-ink">Modifier la ressource</h1>
      <p className="mt-1 text-sm text-ink/50">Mettez à jour les métadonnées, remplacez les PDF ou ajoutez une couverture personnalisée.</p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-6 rounded-xl border border-ink/10 bg-white p-6 shadow-sm">
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium text-ink/75">Résumé anglais
            ...
            {(["fr", "en"] as const).map((language) => {
              ...
              return (
                <div key={language} className="rounded-lg border border-ink/10 p-4">
                  <h3 className="text-sm font-bold text-ink">PDF {language === "fr" ? "français" : "anglais"}</h3>
                  ...
                  ) : <p className="mt-2 text-xs text-ink/40">Aucun fichier actuel</p>}
                  <input
                    type="file"
                  ...
                  />
                  {replacement && <p className="mt-2 truncate text-xs text-[#489e42]">Remplacement : {replacement.name}</p>}
                  {currentPath && (
                    <label className="mt-3 flex items-center gap-2 text-xs text-red-600">
                      <input type="checkbox" checked={removing} onChange={(event) => language === "fr" ? setRemoveFr(event.target.checked) : setRemoveEn(event.target.checked)} /> Supprimer le fichier actuel
                    </label>
                  )}
                </div>
              );
        ...
        <aside className="h-fit rounded-xl border border-ink/10 bg-white p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-sm font-bold text-ink"><ImageIcon className="h-4 w-4 text-[#0072bc]" /> Image de couverture</h2>
          <div className="mt-3 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-lg bg-[#103b5c]">
            {newCoverPreview ? <img src={newCoverPreview} alt="Aperçu de la nouvelle couverture" className="h-full w-full object-contain" /> : form.cover_image_path ? <img src={form.cover_image_path} alt="Couverture actuelle" className="h-full w-full object-contain" /> : <FileText className="h-9 w-9 text-white/30" />}
          </div>
          <p className="mt-2 text-xs leading-relaxed text-ink/45">{form.cover_is_custom ? "Une couverture personnalisée est actuellement utilisée." : "Cette couverture a été générée à partir de la première page PDF disponible."}</p>
          ...
          {form.cover_is_custom && (
            <label className="mt-3 flex items-center gap-2 text-xs text-red-600">
              <input type="checkbox" checked={removeCustomCover} onChange={(event) => { setRemoveCustomCover(event.target.checked); if (event.target.checked) setNewCover(null); }} /> Supprimer la couverture personnalisée et la régénérer
            </label>
          )}
        </aside>
      </div>
